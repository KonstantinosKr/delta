...
void tarch::logging::CommandLineLogger::indent( bool indent, const std::string& trace, const std::string& message ) {
  ...
  if (indent) {
    ...
    #if !defined(SharedMemoryParallelisation)
    if (_indentTraces.empty()) {
      // A matching logTraceIn was lost (e.g. an exception unwound past it).
      // There is nothing to pop, so just warn and keep going.
      std::cerr << "warning: CommandLineLogger::indent(false) called for trace " << trace
                << " without a matching indent(true); message: " << message << std::endl;
    }
    else if (_indentTraces.top()==trace) {
      _indentTraces.pop();
    }
    else {
      // Traces are out of sync (e.g. an exception unwound past a logTraceIn).
      // Resynchronise the stack instead of aborting: drop the stale entries so
      // that the remaining pairs are balanced again.
      std::cerr << "warning: CommandLineLogger::indent trace mismatch: expected "
                << _indentTraces.top() << ", got " << trace
                << "; resynchronising indent stack. message: " << message << std::endl;
      while (!_indentTraces.empty()) {
        _indentTraces.pop();
      }
    }
    #endif
    if (_indent >= NumberOfIndentSpaces) {
      _indent-=NumberOfIndentSpaces;
    }
    else {
      // More logTraceOut calls than logTraceIn calls invoked before. This can
      // happen if an exception unwound past a logTraceIn. Do not abort.
      std::cerr << "warning: CommandLineLogger::indent underflow for trace " << trace
