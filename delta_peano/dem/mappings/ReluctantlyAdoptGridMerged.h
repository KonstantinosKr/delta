...
#ifndef DEM_MAPPINGS_ReluctantlyAdoptGridMerged_H_
#define DEM_MAPPINGS_ReluctantlyAdoptGridMerged_H_
...
namespace dem {
      namespace mappings {
        class ReluctantlyAdoptGridMerged;
      } 
}
 ...
 */
class dem::mappings::ReluctantlyAdoptGridMerged {
  private:
    /**
  ...
  public:
     ...
     */
    ReluctantlyAdoptGridMerged(const ReluctantlyAdoptGridMerged& masterThread);
    #endif

    /**
     * Destructor. Typically does not implement any operation.
     */
    virtual ~ReluctantlyAdoptGridMerged();
     ...
     */   
    void mergeWithWorkerThread(const ReluctantlyAdoptGridMerged& workerThread);
    #endif
 ...
 if ( fineGridVertex.isAdjacentToDomainOf( tarch::parallel::Node::getInstance().getRank() ) ) {
   ...
     ...
     * vertex. Then, it calls enterCell and leaveCell up to @f$ 2^d @f$ times for the 
     ...
     * In the Peano world, an algorithm tells the grid that the grid should be 
     * traversed. The grid then decides how to do this and runs over all cells
     * (and vertices). For each cell, it calls enterCell(), i.e. if you want 
     * your algorithm to do something on a cell, you should implement this 
     * operation. If the cell is refined, it then descends into the finer levels
     * and some time later calls leaveCell().
     *
     * @image html peano/grid/geometry-cell-inside-outside.png
     *
     ...
     * mapping operation or touchVertexFirstTime() or enterCell() are called.
     ...
     *
     * @see ReluctantlyAdoptGridMerged()
     */
    void beginIteration(
     ...
     * Iteration is done
     * 
     * This operation is called at the very end, i.e. after all the leaveCell() 
