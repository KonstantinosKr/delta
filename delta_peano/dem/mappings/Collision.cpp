...
void dem::mappings::Collision::addCollision(
  ...
  for (
    std::vector<Collisions>::iterator p=_collisionsOfNextTraversal[particleA._persistentRecords._globalParticleNumber].begin();
  ...
  for (
    std::vector<Collisions>::iterator p=_collisionsOfNextTraversal[particleB._persistentRecords._globalParticleNumber].begin();
  ...
  for (
    std::vector<delta::collision::contactpoint>::iterator p = newContactPoints.begin();
...
void dem::mappings::Collision::touchVertexFirstTime(
  ...
  for (int i=0; i<fineGridVertex.getNumberOfParticles(); i++) { // No nead to loop over virtual particles here as well
    ...
    if ( _activeCollisions.count(currentParticle._persistentRecords._globalParticleNumber)>0 ) {
      ...
      for (
        std::vector<Collisions>::iterator p = _activeCollisions[currentParticle._persistentRecords._globalParticleNumber].begin();
        p != _activeCollisions[currentParticle._persistentRecords._globalParticleNumber].end();
        p++
      ) {
        delta::collision::filter(
          p->_contactPoints,
          std::min( currentParticle._persistentRecords._hMin, p->_copyOfPartnerParticle._persistentRecords._hMin )
        );
        ...
        // @todo Konstantinos please implement the torque
        currentParticle._persistentRecords._angularVelocity(0) = MoveParticles::timeStepSize * (torque[0] / currentParticle._persistentRecords._mass);
        currentParticle._persistentRecords._angularVelocity(1) = MoveParticles::timeStepSize * (torque[1] / currentParticle._persistentRecords._mass);
        currentParticle._persistentRecords._angularVelocity(2) = MoveParticles::timeStepSize * (torque[2] / currentParticle._persistentRecords._mass);
...
dem::mappings::Collision::Collision(const Collision&  masterThread) {
  logTraceIn( "Collision(Collision)" );
  // @todo Insert your code here
  logTraceOut( "Collision(Collision)" );
}
