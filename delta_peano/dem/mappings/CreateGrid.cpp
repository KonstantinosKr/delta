...
std::vector<delta::geometry::Object::Object>     dem::mappings::CreateGrid::_coarseObjects;
std::vector<delta::geometry::Object::Object>     dem::mappings::CreateGrid::_insitufineObjects;
std::vector<delta::geometry::Object::Object>     dem::mappings::CreateGrid::_fineObjects;
    ...
    iREAL centreAsArray[3])
{
  ...
  for(unsigned i=0; i<_coarseObjects.size(); i++)
  {
    delta::geometry::Object::Object Object = _coarseObjects[i];
    ...
    iREAL centreAsArray[3])
{
  ...
  for(unsigned i=0; i<_insitufineObjects.size(); i++)
  {
    delta::geometry::Object::Object Object = _insitufineObjects[i];
    if((Object.getCentre()[0] >= cellXLeftBoundary && Object.getCentre()[0] <= cellXRightBoundary) &&
       (Object.getCentre()[1] >= cellYDWBoundary && Object.getCentre()[1] <= cellYUPBoundary) &&
       (Object.getCentre()[2] >= cellZLeftBoundary && Object.getCentre()[2] <= cellZRightBoundary))
    {
  ...
  for(unsigned i=0; i<_fineObjects.size(); i++)
  {
    delta::geometry::Object::Object obj = _fineObjects[i];
    ...
    std::vector<delta::geometry::Object::Object> &fineObjects)
{
  ...
  if(octSectTimes && xCoordinates.size() > 0)
  {
    ...
    for(unsigned i=numOfSubParticles; i>(octSectTimes-1)*8; i--)
    {
      ...
      delta::geometry::Object::Object obj("mesh", 0, centroid[0], material, isObstacle, isFriction);
...
)
{
  ...
  for(unsigned i=0; i<xCoordinates.size(); i+=3)
  {
    ...
    delta::geometry::Object obj("mesh", 0, Oarray, material, isObstacle, isFriction);
		...
		dem::State&  solverState)
{
  ...
  if(_scenario[1] == nuclear)
  {
    ...
    delta::geometry::Object::Object ObjectFloor("cube", 0, position, material, isObstacle, isFriction);
    ...
    if(_scenario[0] == sla)
    {
      delta::world::loadNuclearGeometry(pos, width, 1, _fineObjects);
    } else if(_scenario[0] == dla)
    {
      delta::world::loadNuclearGeometry(pos, width, 2, _fineObjects);
    }
    else if(_scenario[0] == nuclearDeck)
    {
      ...
      {
        //nuclear deck 1s
        delta::world::makeBrickGrid(pos, 0.10, 1, 0.1, 1, _fineObjects);
      }else if(_scenario[2] == n4)
      {
        //nuclear deck 4s
        delta::world::makeBrickGrid(pos, 0.10, 2, 0.1, 1, _fineObjects);
      }else if(_scenario[2] == n32)
      {
        //nuclear deck 32s
        delta::world::makeBrickGrid(pos, 0.15, 4, 0.1, 2, _fineObjects);
      }
      else if(_scenario[2] == n64)
      {
        //nuclear deck 64
        delta::world::makeBrickGrid(pos, 0.15, 4, 0.4, 4, _fineObjects);
      }
      else if(_scenario[2] == n256)
      {
        //nuclear deck 256
        delta::world::makeBrickGrid(pos, 0.15, 10, 0.08, 4, _fineObjects);
      }
    }
  } else if(_scenario[1] == hopper)
  {
...
#if true
    delta::geometry::Object::Object ObjectHopper("hopper", 0, position, material, isObstacle, isFriction);
    ...
    for(unsigned i=0; i<grid.size(); i++)
    {
      ...
      if(_isSphere)
      {
        delta::geometry::Object particles("sphere", i, p, material, isObstacle, isFriction);
        std::array<double, 3> l = {0, -1, 0};
        particles.setLinearVelocity(l);
        _insitufineObjects.push_back(particles);
      } else {
        delta::geometry::Object particles("granulate", 0, p, material, isObstacle, isFriction);
        std::array<double, 3> l = {0, -1, 0};
        particles.setLinearVelocity(l);
        _insitufineObjects.push_back(particles);
      }
    ...
    {
      delta::world::uniformlyDistributedTotalMass(totalMass, hopperParticles, _isSphere, _noPointsPerParticle, _insitufineObjects);
    }
    else if(_scenario[2] == nonuniform)
    {
      iREAL subcellx = ((double)subGridLength/(double)xzcuts) - _epsilon*4;
      delta::world::nonUniformlyDistributedTotalMass(totalMass, hopperParticles, _isSphere, subcellx, _noPointsPerParticle, _insitufineObjects);
    }
  ...
  } else if(_scenario[1] == friction)
  {
    ...
    ////////////////////////////////////////////////////////////////////////////////
    delta::geometry::Object ObjectFloor("cube", 0, position, material, isObstacle, isFriction);
    ...
    if(_isSphere){
      delta::geometry::Object ObjectA("cube", 1, position, material, isObstacle, isFriction);
      ObjectA.generateMesh(rad, rad, rad, 0, 0, 0, rad, _noPointsPerParticle);
      _coarseObjects.push_back(ObjectA);
    } else {
      delta::geometry::Object ObjectA("sphere", 1, position, material, isObstacle, isFriction);
  ...
  } else if(_scenario[0] == TwoParticlesCrash)
  {
    ...
    if(_isSphere){
      delta::geometry::Object ObjectA("sphere", 0, centreArray, material, false, false);
      ...
      delta::geometry::Object ObjectB("sphere", 1, centreArray, material, false, false);
      ...
      centreArray[1] = 0.5;
      centreArray[2] = 0.5;
      delta::geometry::Object ObjectA("granulate", 0, centreArray, material, false, false);
      ...
      delta::geometry::Object ObjectB("granulate", 1, centreArray, material, false, false);
          ...
          _scenario[0] == freefallWithRandomOrientedCubes)
  {
    ...
    if(_isSphere){
      delta::geometry::Object ObjectA("sphere", 0, position, material, false, false);
      ObjectA.generateSphere(rad);
      ObjectA.setLinearVelocity(linear);
      _fineObjects.push_back(ObjectA);
    } else {
      delta::geometry::Object ObjectA("cube", 0, position, material, false, false);
      ObjectA.generateMesh(rad,rad,rad,0,0,0,rad,_noPointsPerParticle);
      _fineObjects.push_back(ObjectA);
      ObjectA.setLinearVelocity(linear);
    }
