  ...
  if (
    ...
    dfor(k,particlesInCellPerAxis) {
      const tarch::la::Vector<DIMENSIONS,double> centre =
        fineGridVerticesEnumerator.getVertexPosition() + _particleDiamMax * (k.convertScalar<double>()+0.5);
      double particleDiameter =
        (_particleDiamMin + (_particleDiamMax-_particleDiamMin) * (static_cast<double>( rand() ) / static_cast<double>(RAND_MAX)) ) / std::sqrt( DIMENSIONS );
      ...
      const int     newParticleNumber = vertex.createNewParticle( centre );
      ...
      #ifdef Dim2
      double centreAsArray[] = {centre(0),centre(1)};

      if (UseCubesAndSquares) {
        ttd::PrimitiveGenerator::generateSquare(
          centreAsArray,
          particleDiameter,
          static_cast<double>( rand() ) / static_cast<double>(RAND_MAX),
          numberOfTriangles, x, y
        );

        assertionEquals( numberOfTriangles, 4 );
      }
      ...
      else if (UseAlignedRotatedCubesAndSquares) {
        ttd::PrimitiveGenerator::generateSquare(
          centreAsArray,
          particleDiameter,
          1.0,
        ...
        ttd::PrimitiveGenerator::generateParticle(
          centreAsArray,
          particleDiameter,
          numberOfTriangles, x, y
        );
      ...
      vertex.getParticle(newParticleNumber)._persistentRecords._hMin                 = delta::primitives::computeHMin(
        xCoordinates,
        yCoordinates,
        zCoordinates
      );
