...
std::vector<delta::contact::contactpoint> delta::contact::detection::gjk(
  ...
  iREAL  minDistance = std::numeric_limits<iREAL>::max();
  ...
  iREAL *a = new iREAL[numberOfTrianglesOfGeometryA*3];
  iREAL *b = new iREAL[numberOfTrianglesOfGeometryB*3];
  iREAL p[3], q[3];
  
  int na = 0;
  for(int i=0;i<numberOfTrianglesOfGeometryA;i++)
  {
    a[na] = xCoordinatesOfPointsOfGeometryA[i];
    a[na+1] = yCoordinatesOfPointsOfGeometryA[i];
    a[na+2] = zCoordinatesOfPointsOfGeometryA[i];
    na += 3;
  }
  
  int nb = 0;
  for(int i=0;i<numberOfTrianglesOfGeometryB;i++)
  {
    b[nb] = xCoordinatesOfPointsOfGeometryB[i];
    b[nb+1] = yCoordinatesOfPointsOfGeometryB[i];
    b[nb+2] = zCoordinatesOfPointsOfGeometryB[i];
    nb += 3;
  }
  
  delta::contact::detection::gjk(a, na, b, nb, p, q);
...
iREAL delta::contact::detection::gjk (iREAL *a, int na, iREAL *b, int nb, iREAL *p, iREAL *q)
{
  point w [4];
  iREAL v [3], vlen, delta, mi = 0.0, l [4];

  int toofar = 1, n = 0, j = 0, k = (na+nb)*(na+nb);
