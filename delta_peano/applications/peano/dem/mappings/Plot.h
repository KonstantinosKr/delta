...
class dem::mappings::Plot {
 ...
 if ( fineGridVertex.isAdjacentToDomainOf( tarch::parallel::Node::getInstance().getRank() ) ) {
   ...
     ...
     * Iteration is done
     * 
     * This operation does close all the plotters. Before, however, it plots
     * all the contact points that have been identified in this sweep. One
     * might think that the code should run over
     * Collision::_collisionsOfNextTraversal. But please keep in mind that this
     * endIteration() is called after the collision mapping's endIteration()
     * which has swapped the arrays already.
     */
    void endIteration(
      dem::State&  solverState
    );
